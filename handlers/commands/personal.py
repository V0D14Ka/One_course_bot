...
async def form_set(message: types.Message, state: FSMContext, **kwargs):
    async with state.proxy() as data:
        ...
        await message.delete()
        await call.message.edit_text(messages.ask_for_update_user_info % ("3", "Номер группы", "'Б9999-11.11.11'"))
        await FSMUpdateUserInfo.next()
...
async def personal(message: Union[types.CallbackQuery, types.Message], changed=False):
...
async def menu_navigate(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    match current_level:
        ...
        case "1":
            if category == "2":
                await call.message.edit_text(messages.ask_for_update_user_info % ("1", "ФИО", "'Иванов Иван Иванович'"))
                async with state.proxy() as data:
                    # Передаем необходимую информацию
                    data["category"] = category
                    data["call"] = call
                    await FSMUpdateUserInfo.full_name.set()
            else:
                try:
